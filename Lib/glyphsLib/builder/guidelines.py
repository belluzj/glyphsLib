...
# TO REVIEW: (jany) Ideally, each feature that can be translated between
#     UFO and Glyphs will have a rather small file like this one, with
#     two main functions,
#       `to_ufo_...(context, ...)` and
#       `to_glyphs_...(context, ...)`
#     where each does the opposite of the other.
...
def to_glyphs_guidelines(context, glyphs_obj, ufo_obj):
    ...
    for guideline in ufo_obj.guidelines:
        new_guideline = context.classes.GSGuideLine()
