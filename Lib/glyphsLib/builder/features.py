...
# TO REVIEW: (jany) This is an attempt to remove some code from "to_ufos"
#     by moving some code in more specific modules and by removing intermediary
#     tuples, and using the objects directly instead.
def to_ufo_features(context):
    for ufo in context.ufos.values():
        add_features_to_ufo(context, ufo)
...
def add_features_to_ufo(context, ufo):
