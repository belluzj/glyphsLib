...
def to_ufos(font, include_instances=False, family_name=None, debug=False,
            defcon=defcon):
    # TODO: (jany) update the documentation
    ...
    first_ufo = next(iter(ufos.values()))
...
def to_glyphs(ufos, designspace=None, classes=classes):
    ...
    for index, ufo in enumerate(ufos):
        ...
        to_glyphs_font_attributes(context, ufo, master, is_initial=(index == 0))
